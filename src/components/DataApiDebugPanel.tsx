...
const STORAGE_HISTORY = "trb.dbApiDebug.history";
...
export default function DataApiDebugPanel() {
  ...
  const applyMethod = useCallback(
    (nextMethod: DbApiGrpcMethod) => {
      ...
      setError("");
      setParseError("");
      writeStorage(STORAGE_METHOD, nextMethod);
    },
    [bodyText, method],
  );
  ...
  const restoreHistory = (item: HistoryItem) => {
    const nextMethod = isMethod(item.method) ? item.method : method;
    applyMethod(nextMethod);
    setBodyText(item.requestText);
    writeStorage(STORAGE_BODY + nextMethod, item.requestText);
  };
        ...
        <div className="debug-services">
          {DB_API_GRPC_METHODS.map((item) => (
            <button
              key={item.value}
              type="button"
              className={`debug-chip${item.value === method ? " is-active" : ""}`}
              onClick={() => applyMethod(item.value)}
              disabled={busy}
            >
